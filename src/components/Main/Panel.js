...
import {customIndicatorById, customLegendById} from '../../js/customIndicators';
...
export const Panel = () => {
    ...
    const [autoClassificationMethod, setAutoClassificationMethod] = useState(CLASSIFICATION_EQUAL_INTERVALS);
    const [autoClassificationColorCount, setAutoClassificationColorCount] = useState(5);
    const [autoClassificationColorScale, setAutoClassificationColorScale] = useState();
    const [circleLegend, setCircleLegend] = useState({isEnabled: false, size: [5, 600], opacity: 0.7, strokeWidth: 1, color: {r: 200, g: 200, b: 200, a: 0.8}, stroke: {r: 0, g: 0, b: 0, a: 1}});
    const [tabSelected, handleTabSelectedChange] = useState('rates');

    const [isCoverageRateDiffEnabled, setCoverageRateDiffEnabled] = useState(false);
    const [isCoverageRateCatchUpEnabled, setCoverageRateCatchUpEnabled] = useState(false);
    ...
    const generateLegendRef = (indicator, organisationUnitLevel) => {
        ...
        if (!isLegendAuto(legend)) {
            const testRef = data.legendSets.legendSets.filter(o => o.id === legend)
            return { items: _.isEmpty(testRef) ? [] : _.sortBy(testRef[0].legends, ['startValue']) };
        }
        ...
        const sortedValues = values.filter(_.isNumber).sort(function(a, b){return a-b})
        const autoLegend = getAutomaticLegendItems(sortedValues, autoClassificationMethod, autoClassificationColorScale)

        return { items: _.sortBy(autoLegend, ['startValue']) };
    }

    const generateCircleLegendRef = (organisationUnitLevel) => {
        const dataElementId = circleLegend.dataElement && circleLegend.dataElement.id;
        ...
        let max, min;
        _.each(dataElementObject[dataElementId][organisationUnitLevel], (data, period) => {
    ...
    const [legendRefsState, dispatch] = useReducer(legendRefsReducer, {
        needsRefresh: false,
    }, init => ({
        ...init,
        legendRef: generateLegendRef(indicator, organisationUnitLevel),
    ...
    return (
        <main id="main">
            <div id="grid-root">
                <Grid direction="row" container spacing={3}>
                    <Grid item xs={3}>
                        <Card className="padded-card controls-card">
                            ...
                            <Box component="div" display="block">
                                <FormControl>
                                    <InputLabel>Health programme</InputLabel>
                                    <Select
                                        defaultValue={data.indicatorGroups.indicatorGroups[0].id}
                                        onChange={evt => {
                            ...
                            <Tabs
                                ...
                                textColor="primary"
                                onChange={(event, value) => handleTabSelectedChange(value)}
                            >
                                <Tab label={<Box display="flex" alignItems="center"><Texture /> Rates</Box>} value="rates" />
                                <Tab label={<Box display="flex" alignItems="center"><Adjust /> Absolute figures</Box>} value="absolutes"/>
                            </Tabs>
                            <TabPanel value={tabSelected} index="rates">
                                <Box display="flex" alignItems="center">
                                    <Box mr={3}>Enable</Box>
                                    <Checkbox onChange={() => setCoverageLayerEnabled(!isCoverageLayerEnabled)}
                                ...
                                { isCoverageLayerEnabled && <>
                                    ...
                                    <FormControl>
                                        <Box display="flex" alignItems="center" mt={2}>
                                            ...
                                            <Slider
                                                ...
                                                onChange={(event, value) => toggleCoverageLayerOpacity(value)}
                                    ...
                                    <Box component="div" display="block">
                                        <IndicatorSelector
                                            ...
                                            groups={
                                                data.indicatorGroups.indicatorGroups
                                            }
                                            onChange={indicator => {
                                    ...
                                    <Box component="div" display="block">
                                        <LegendSelector
                                            allowAutoLegend={!isCustomIndicator()}
                                            legend={legend}
                                    ...
                                    {isLegendAuto(legend) &&
                                    <Box component="div" display="block">
                                        {legendRefsState.needsRefresh && <IconButton className="refresh-legend-button"
                                                                                     onClick={() => dispatch('forceRefresh')}><RefreshIcon/>Recalculate
                                            legend</IconButton>}
                                        <Classification
                                            classes={autoClassificationColorCount}
                                            method={autoClassificationMethod}
                                            colorScale={autoClassificationColorScale}
                                            setClassification={value => {
                                                setAutoClassificationMethod(value);
                                                dispatch('forceRefresh');
                                            }}
                                            setColorScale={value => {
                                                setAutoClassificationColorScale(value);
                                                setAutoClassificationColorCount(value.split(',').length);
                                                dispatch('forceRefresh');
                                            }}/>
                                    </Box>
                                }
                                    ...
                                    <AccordionDetails>
                                        <FormGroup>
                                            <FormControlLabel
                                                ...
                                                control={<Checkbox onChange={() => setCoverageRateDiffEnabled(!isCoverageRateDiffEnabled)}
                                                         checked={isCoverageRateDiffEnabled}
                                                         color="primary"/>}
                                            />
                                            { isCoverageRateDiffEnabled && <FormControlLabel
                                                label="Coverage rate catch-up trend"
                                                control={<Checkbox onChange={() => setCoverageRateCatchUpEnabled(!isCoverageRateCatchUpEnabled)}
                                                         checked={isCoverageRateCatchUpEnabled}
                                                         color="primary"/>}
                                            />}
                                        </FormGroup>
                                    </AccordionDetails>
                    ...
                    <Grid item xs={9}>
                        <Card className="padded-card">
                            ...
                            <Grid id="main-map" direction="row" container spacing={3}>
                                <Grid item xs={3}>
                                    <div className="layer-card">
                                        <Box display="flex" alignItems="center">
                                            <Box flexGrow={1}>{currentPeriod.label}</Box>
                                            <Box className="boxed" data-html2canvas-ignore>
                                                <IconButton aria-label="export" title="Export PDF" size="small"
                                                            onClick={() => {
                                                                window.scrollTo(0, 0);
                                                                html2canvas(document.querySelector("#main-map"), {allowTaint: true, useCORS: true}).then(canvas => {
                                                                    canvas.toBlob(function (blob) {
                                                                        saveAs(blob, "wholePage.png");
                                                                    });
                                                                });
                                                            }}>
                                                    <Box ml={1} mr={1}>Export</Box> <InsertDriveFileOutlined/>
                                                </IconButton>
                                            </Box>
                                        </Box>
                                        <Box mb="5px">
                                        ...
                                        <Box>
                                            <Typography gutterBottom>{currentIndicator.length > 0 ? currentIndicator[0].name : ''}</Typography>
                                            <Divider/>
                                            <Box mt={1}>
                                                <CardContent
                                                ...
                                                >
                                                    <div className="layer-card-legend">
                                                        <Legend {...legendRefsState.legendRef} />
                                                    </div>
                                                </CardContent>
                                            </Box>
                                        </Box>
                                        {circleLegend.isEnabled && <EagerCircleLegend
                                            circleLegend={circleLegend}
                                            circleLegendRef={legendRefsState.circleLegendRef}
                                            timeScale={timeScale}
                                            ouLevel={organisationUnitLevel}
                                            isReady={circleLegendIsReadyCallback}
                                        />}
                                    </div>
                                </Grid>
                                <Grid item xs={9}>
                                    <EagerMap
                                        ...
                                        circleLegendRef={legendRefsState.circleLegendRef}
                                        isCoverageLayerEnabled={isCoverageLayerEnabled}
                                        coverageLayerOpacity={coverageLayerOpacity}
                ...
                {!isCustomIndicator() && <Grid direction="row" container spacing={2}>
                    <Grid item xs={6}>
                        <Card className="padded-card">
                            <div>
                                <EagerChart
                                    ouLevel={data.organisationUnitLevels.organisationUnitLevels[0].level}
                                    ou={data.nationalUnits.organisationUnits[0].id}
                                    dx={indicator}
                ...
                { isCoverageRateDiffEnabled &&
                    <CalendarGraphs isCoverageRateCatchUpEnabled={isCoverageRateCatchUpEnabled} />
                }
            </div>
        </main>
    )
